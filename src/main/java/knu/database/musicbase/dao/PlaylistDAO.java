...
public class PlaylistDAO extends BasicDataAccessObjectImpl<Playlist, Long> {
        ...
        try (Connection conn = getConnection();
            ...
            try (ResultSet generatedKeys = pstmt.getGeneratedKeys()) {
                if (generatedKeys.next()) {
                    ...
                    return new Playlist(generatedId, entity.getTitle(), entity.getIsCollaborative(),
                            entity.getUserId());
                } else {
    ...
    public Optional<Playlist> findById(Long id) {
        ...
        return executeQueryOne(sql, this::mapResultSetToPlaylist, id);
    }

    @Override
    public List<Playlist> findAll() {
        String sql = "SELECT * FROM PLAYLISTS";
        return executeQuery(sql, this::mapResultSetToPlaylist);
    }
    ...
    public List<Playlist> findEditable(long userId) {
        ...
        return executeQuery(sql, this::mapResultSetToPlaylist, userId, userId);
    }
    ...
    public List<Playlist> findByUserId(long userId) {
        ...
        return executeQuery(sql, this::mapResultSetToPlaylist, userId);
    }
     ...
     *
     *
     * 변경 : "SELECT S.SONG_ID, S.TITLE AS SONG_TITLE, S.PLAY_LINK, A.NAME AS
     * ARTIST_NAME " +
    ...
    public List<SongDetail> findPlaylistDetailsByPlaylistId(long playlistId) {
        ...
        return executeQuery(sql, rs -> new SongDetail(
                rs.getLong("SONG_ID"),
                rs.getString("SONG_TITLE"),
                rs.getString("PLAY_LINK"),
                rs.getString("ARTIST_NAME")), playlistId);
    }
    ...
    public List<Playlist> searchPlaylists(String title, boolean titleExact,
        ...
        if (songCountMin != null) {
            havingConditions.add("COUNT(DISTINCT co.Song_id) >= ?");
        ...
        if (commentCountMin != null) {
            havingConditions.add("COUNT(DISTINCT c.Comment_id) >= ?");
            params.add(commentCountMin);
        }
        ...
        if (lenMin != null) {
            havingConditions.add("NVL(SUM(s.Length), 0) >= ?");
            params.add(lenMin);
        }
        if (lenMax != null) {
            havingConditions.add("NVL(SUM(s.Length), 0) <= ?");
        ...
        return executeQuery(sql.toString(), this::mapResultSetToPlaylist, params.toArray());
    }
